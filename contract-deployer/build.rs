...
fn main() {
    ...
    core_mod_contents.push_str("pub use self::allocationmanager::AllocationManager::OperatorSet;\n");
...
fn add_imports_to_file(file_path: &str, contract: &str) {
    ...
    let mut contents = String::new();
    file.read_to_string(&mut contents).expect(&format!("Failed to read {}", file_path));
    
    // Add the imports at the top
    let new_contents = format!("#![allow(elided_lifetimes_in_paths)]\nuse {}::*;\n\n{}", contract, contents);
    
    // Write back to the file
    let mut file = fs::File::create(path).expect(&format!("Failed to create {}", file_path));
    file.write_all(new_contents.as_bytes()).expect(&format!("Failed to write to {}", file_path));
}
