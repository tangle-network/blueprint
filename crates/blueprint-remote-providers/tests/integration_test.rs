...
mod tests {
    use blueprint_remote_providers::{
        ResourceSpec, CloudProvider, DeploymentTracker, UnifiedInfrastructureProvisioner,
        PricingService, HealthMonitor, HealthStatus, RemoteDeploymentExtensions,
    };
    use std::sync::Arc;
    ...
    async fn test_deployment_tracker() {
        ...
        tracker.register_deployment("blueprint-123".to_string(), record.clone()).await.unwrap();
    ...
    async fn test_health_monitor() {
        ...
        let monitor = HealthMonitor::new(provisioner, tracker.clone())
            .with_config(Duration::from_secs(5), 3, false);
        ...
        tracker.register_deployment("test-deployment".to_string(), record).await.unwrap();
    ...
    async fn test_remote_deployment_extensions() {
        ...
        extensions.event_handler.on_service_initiated(
            100, // blueprint_id
            1,   // service_id
            Some(config),
        ).await.unwrap();
        ...
        extensions.event_handler.on_service_terminated(100, 1).await.unwrap();
    ...
    async fn test_ttl_expiry() {
        use blueprint_remote_providers::manager_integration::{RemoteDeploymentRegistry, TtlManager};
    ...
    fn test_health_status_mapping() {
        ...
        assert_eq!(health_from_instance(InstanceStatus::Running), HealthStatus::Healthy);
        assert_eq!(health_from_instance(InstanceStatus::Starting), HealthStatus::Degraded);
        assert_eq!(health_from_instance(InstanceStatus::Stopped), HealthStatus::Unhealthy);
        assert_eq!(health_from_instance(InstanceStatus::Terminated), HealthStatus::Unhealthy);
        assert_eq!(health_from_instance(InstanceStatus::Unknown), HealthStatus::Unknown);
    }
}