...
fn test_plaintext_credential_storage_vulnerability() {
    ...
    assert!(
        creds.access_key.is_some(),
        "AWS access key stored in plaintext!"
    );
    assert!(
        creds.secret_key.is_some(),
        "AWS secret key stored in plaintext!"
    );
    assert!(
        creds.api_token.is_some(),
        "DigitalOcean API token stored in plaintext!"
    );
    assert!(
        creds.api_key.is_some(),
        "Vultr API key stored in plaintext!"
    );
...
fn test_insecure_api_authentication_patterns() {
    ...
    assert!(
        ...
        "No AWS signature authentication!"
    );
    assert!(
        !url.contains("X-Amz-Date"),
        "No timestamp in authentication!"
    );
    ...
    assert!(
        do_auth_header.contains(do_token),
        "API token transmitted in header!"
    );
...
fn test_cloud_credential_leakage_vulnerabilities() {
    ...
    assert!(
        log_entries.iter().any(|log| log.contains("AKIA")),
...
fn test_comprehensive_cloud_api_attack_chain() {
    ...
    assert!(
        vulnerable_combo.len() > 5,
        "Multiple critical vulnerabilities enable attack chain!"
    );
}
