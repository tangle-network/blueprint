...
async fn test_cloud_provisioner_initialization() {
    ...
    assert!(provisioner.is_ok(), "CloudProvisioner should initialize successfully");
    ...
    let result = provisioner.provision(CloudProvider::AWS, &resource_spec, "us-east-1").await;
    ...
    match result.unwrap_err() {
        ...
        other => {
            println!("✓ Provider initialization returned expected error: {:?}", other);
        }
    }
}
...
async fn test_digitalocean_adapter_integration() {
    ...
    match result {
        Ok(instance) => {
            println!("✓ Successfully provisioned DigitalOcean instance: {}", instance.id);
            ...
            let deployment_result = adapter
                .deploy_blueprint_with_target(&deployment_target, BLUEPRINT_IMAGE, &resource_spec, env_vars)
                .await;
            ...
            match deployment_result {
                        ...
                        match health_check {
                            Ok(healthy) => {
                                if healthy {
                                    ...
                                    println!("⚠ QoS endpoint not healthy (expected for test environment)");
                                }
                            }
                            Err(e) => {
                                println!("⚠ QoS health check failed: {} (expected for test environment)", e);
                            }
                        }
                    }
                ...
                Err(e) => {
                    println!("⚠ Blueprint deployment failed: {} (expected without proper SSH setup)", e);
                }
            }
        ...
        Err(e) => {
            println!("⚠ Instance provisioning failed: {} (may be expected due to API limits or configuration)", e);
        }
    }
}
...
async fn test_multi_provider_deployment() {
    ...
    for (provider, region) in providers_to_test {
        ...
        let result = provisioner.provision(provider.clone(), &resource_spec, region).await;
        
        match result {
            Ok(instance) => {
                println!("✓ Successfully provisioned instance on {}: {}", provider, instance.id);
    ...
    for (provider, instance) in &results {
        println!("Testing deployment to {} instance: {}", provider, instance.id);
    ...
    for target in deployment_targets {
        ...
        match target {
            DeploymentTarget::ManagedKubernetes { cluster_id, namespace } => {
                println!("✓ Managed Kubernetes target configured: {} / {}", cluster_id, namespace);
            }
            DeploymentTarget::GenericKubernetes { context, namespace } => {
                println!("✓ Generic Kubernetes target configured: {:?} / {}", context, namespace);
            }
            _ => {}
        }
    }
...
async fn test_qos_remote_integration() {
    ...
    assert!(port_mappings.contains_key(&9615), "QoS metrics port should be exposed");
    assert!(port_mappings.contains_key(&9944), "QoS RPC port should be exposed");
    ...
    assert!(qos_metrics_url.contains("9615"), "Metrics URL should use port 9615");
    assert!(qos_health_url.contains("9615"), "Health URL should use port 9615");
