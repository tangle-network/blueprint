...
fn test_container_security_configuration_vulnerabilities() {
    ...
    assert!(!docker_cmd.contains("--privileged false"), "Privileged mode not explicitly disabled!");
    assert!(!docker_cmd.contains("--pid-limit"), "No process limit restrictions!");
    assert!(!docker_cmd.contains("--ulimit"), "No resource ulimits!");
    assert!(!docker_cmd.contains("--device"), "Device access not restricted!");
    
    // Network exposure issues
    assert!(docker_cmd.contains("0.0.0.0"), "Services exposed on all interfaces!");
...
fn test_container_runtime_privilege_vulnerabilities() {
    ...
    assert!(!container_command.contains("--user 1000:1000"), "Container runs as root!");
}
...
fn test_container_network_security_vulnerabilities() {
    ...
    assert!(!network_bindings.iter().any(|b| b.contains("127.0.0.1")), 
...
fn test_container_filesystem_security_vulnerabilities() {
    ...
    assert!(!docker_cmd.contains("--read-only"), "Filesystem is writable!");
    assert!(!docker_cmd.contains("--tmpfs /tmp"), "No tmpfs for temporary files!");
    assert!(!docker_cmd.contains("--tmpfs /var/run"), "No tmpfs for runtime files!");
    ...
    assert!(!docker_cmd.contains("--read-only"), "Writable filesystem vulnerability!");
}
...
fn test_container_resource_limit_vulnerabilities() {
    ...
    assert!(!docker_cmd.contains("--pids-limit"), "No process count limit!");
    assert!(!docker_cmd.contains("--ulimit nproc"), "No user process limit!");
    assert!(!docker_cmd.contains("--ulimit nofile"), "No file descriptor limit!");
    assert!(!docker_cmd.contains("--ulimit fsize"), "No file size limit!");
    assert!(!docker_cmd.contains("--memory-swappiness"), "No swap control!");
    assert!(!docker_cmd.contains("--oom-kill-disable"), "No OOM protection!");
    assert!(!docker_cmd.contains("--kernel-memory"), "No kernel memory limit!");
    ...
    assert!(!docker_cmd.contains("--pids-limit"), "Process limit vulnerability!");
}
...
fn test_container_capability_privilege_vulnerabilities() {
    ...
    assert!(!docker_cmd.contains("--cap-drop ALL"), "All capabilities available!");
    assert!(!docker_cmd.contains("--cap-add"), "No capability whitelisting!");
    assert!(!docker_cmd.contains("--security-opt no-new-privileges"), "Privilege escalation allowed!");
    assert!(!docker_cmd.contains("--security-opt apparmor"), "No AppArmor protection!");
    assert!(!docker_cmd.contains("--security-opt seccomp"), "No seccomp restrictions!");
    ...
    assert!(!docker_cmd.contains("--cap-drop ALL"), "Excessive privileges vulnerability!");
}
...
fn test_container_image_security_vulnerabilities() {
    ...
    assert!(image_name.contains("latest"), "Using unpinned 'latest' tag!");
    assert!(!image_name.starts_with("sha256:"), "No content-addressable image!");
...
fn test_comprehensive_container_attack_chain() {
    ...
    assert!(insecure_container.contains("0.0.0.0"), "Container security attack chain possible!");
}
