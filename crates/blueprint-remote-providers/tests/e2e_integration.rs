...
use blueprint_remote_providers::{CloudProvider, ResourceSpec};
use std::sync::Arc;
...
async fn test_e2e_deployment_flow() {
    ...
    let status = extensions
        .get_deployment_status("test-blueprint", "test-service")
        .await
        .unwrap();
...
<TerminateInstancesResponse xmlns="http://ec2.amazonaws.com/doc/2016-11-15/">
    ...
    // Track costs
    let cost_report = blueprint_remote_providers::PricingService::new().calculate_cost(
    ...
    // Cleanup
    extensions
        .cleanup_deployment("test-blueprint", "test-service")
        .await
        .unwrap();

    // Verify termination was called
    let requests = mock_server.received_requests().await.unwrap();
    let has_terminate = requests
        .iter()
        .any(|r| r.body_string().contains("TerminateInstances"));
