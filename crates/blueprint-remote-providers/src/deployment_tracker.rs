...
impl DeploymentTracker {
    ...
    pub async fn register_deployment(
        ...
        info!("Registering deployment for Blueprint instance: {}", blueprint_id);
    ...
    pub async fn handle_termination(&self, blueprint_id: &str) -> Result<()> {
        info!("Handling termination for Blueprint instance: {}", blueprint_id);
        
        let deployments = self.deployments.read().await;
        let deployment = deployments.get(blueprint_id)
            .ok_or_else(|| Error::ConfigurationError(format!("No deployment found for {}", blueprint_id)))?
            .clone();
        drop(deployments);
    ...
    pub async fn handle_ttl_expiry(&self, blueprint_id: &str) -> Result<()> {
        info!("Handling TTL expiry for Blueprint instance: {}", blueprint_id);
        ...
        if let Some(deployment) = deployments.get(blueprint_id) {
            ...
            if let Some(expiry) = deployment.expires_at {
                if now >= expiry {
                    ...
                    debug!("TTL not yet expired for {} (expires at {})", blueprint_id, expiry);
                }
            }
        }
    ...
    async fn cleanup_deployment(&self, blueprint_id: &str, deployment: &DeploymentRecord) -> Result<()> {
        info!("Cleaning up deployment: {} (type: {:?})", blueprint_id, deployment.deployment_type);
        
        let handlers = self.cleanup_handlers.read().await;
        let handler = handlers.get(&deployment.deployment_type)
            .ok_or_else(|| Error::ConfigurationError(format!("No handler for {:?}", deployment.deployment_type)))?;
            ...
            match handler.cleanup(deployment).await {
                Ok(_) => {
                    ...
                    if let Some(webhook) = &deployment.cleanup_webhook {
                        self.send_cleanup_notification(webhook, blueprint_id, "success").await;
                    }
                ...
                Err(e) => {
                    attempts += 1;
                    error!("Cleanup attempt {} failed for {}: {}", attempts, blueprint_id, e);
                    
                    if attempts >= max_attempts {
                        // Send failure notification
                        if let Some(webhook) = &deployment.cleanup_webhook {
                            self.send_cleanup_notification(webhook, blueprint_id, "failed").await;
                        }
                        return Err(e);
                    }
    ...
    async fn register_default_handlers(&mut self) {
        ...
        handlers.insert(DeploymentType::LocalKubernetes, Box::new(LocalKubernetesCleanup));
        handlers.insert(DeploymentType::LocalHypervisor, Box::new(LocalHypervisorCleanup));
        ...
        handlers.insert(DeploymentType::DigitalOceanDroplet, Box::new(DigitalOceanCleanup));
    ...
    pub async fn check_all_ttls(&self) -> Result<()> {
        ...
        let expired: Vec<String> = deployments.iter()
    ...
    pub async fn update_instance_id(&self, old_id: &str, new_id: &str) -> Result<()> {
        ...
        if let Some(mut deployment) = deployments.remove(old_id) {
            deployment.resource_ids.insert("instance_id".to_string(), new_id.to_string());
        ...
        if let Some(container_id) = deployment.resource_ids.get("container_id") {
            ...
            if !output.status.success() {
                ...
                if !stderr.contains("No such container") {
                    return Err(Error::ConfigurationError(format!("Docker rm failed: {}", stderr)));
                }
            }
        }
...
impl CleanupHandler for LocalKubernetesCleanup {
    async fn cleanup(&self, deployment: &DeploymentRecord) -> Result<()> {
        let namespace = deployment.resource_ids.get("namespace").map(|s| s.as_str()).unwrap_or("default");
        ...
        if let Some(pod_name) = deployment.resource_ids.get("pod") {
            ...
            if !output.status.success() {
                ...
                if !stderr.contains("NotFound") {
                    return Err(Error::ConfigurationError(format!("kubectl delete failed: {}", stderr)));
                }
            }
        }
    ...
    async fn cleanup(&self, deployment: &DeploymentRecord) -> Result<()> {
        #[cfg(feature = "gcp")]
        {
            ...
            if let (Some(project), Some(zone)) = 
                (deployment.metadata.get("project_id"), deployment.region.as_ref()) {
                
                let provisioner = GcpInfrastructureProvisioner::new(
                    project.clone(),
                    zone.rsplit_once('-').map(|r| r.0).unwrap_or(zone).to_string(),
                    zone.clone(),
                ).await?;
    ...
    async fn cleanup(&self, deployment: &DeploymentRecord) -> Result<()> {
        #[cfg(feature = "azure")]
        {
            ...
            if let (Some(subscription), Some(rg)) = 
                (deployment.metadata.get("subscription_id"), deployment.metadata.get("resource_group")) {
...
impl CleanupHandler for DigitalOceanCleanup {
    async fn cleanup(&self, deployment: &DeploymentRecord) -> Result<()> {
        ...
        if let Some(api_token) = deployment.metadata.get("api_token") {
            ...
            let provisioner = DigitalOceanProvisioner::new(api_token.clone(), region.clone()).await?;
    ...
    async fn cleanup(&self, deployment: &DeploymentRecord) -> Result<()> {
        #[cfg(feature = "aws-eks")]
        {
            ...
            if let Some(cluster_name) = deployment.resource_ids.get("cluster_name") {
                ...
                let nodegroups = eks.list_nodegroups()
                    .cluster_name(cluster_name)
                    .send()
                    .await?;
                
                for ng in nodegroups.nodegroups().unwrap_or_default() {
                    let _ = eks.delete_nodegroup()
                        .cluster_name(cluster_name)
                        .nodegroup_name(ng)
                        .send()
                        .await;
                ...
                eks.delete_cluster()
                    ...
                    .send()
                    .await
                    .map_err(|e| Error::ConfigurationError(format!("Failed to delete EKS: {}", e)))?;
            }
        }
...
impl CleanupHandler for SshCleanup {
    async fn cleanup(&self, deployment: &DeploymentRecord) -> Result<()> {
        use crate::ssh_deployment::{SshDeploymentClient, SshConnection, ContainerRuntime, DeploymentConfig, RestartPolicy};
        
        if let (Some(host), Some(user)) = 
            (deployment.metadata.get("ssh_host"), deployment.metadata.get("ssh_user")) {
            
            let connection = SshConnection {
                host: host.clone(),
                port: deployment.metadata.get("ssh_port")
...
mod tests {
    ...
    async fn test_deployment_registration() {
        ...
        tracker.register_deployment("blueprint-123".to_string(), record).await.unwrap();
    ...
    async fn test_ttl_expiry() {
        ...
        tracker.register_deployment("blueprint-ttl".to_string(), record).await.unwrap();
