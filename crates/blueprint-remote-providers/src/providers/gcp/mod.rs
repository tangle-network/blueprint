...
impl GcpProvisioner {
    ...
    pub async fn provision_instance(
        ...
        info!("Provisioning GCP instance type {} in {}", instance_selection.instance_type, zone);
        ...
        if !response.status().is_success() {
            ...
            return Err(Error::ConfigurationError(format!("GCP API error: {}", error_text)));
        }
        
        let operation: serde_json::Value = response.json().await
            .map_err(|e| Error::ConfigurationError(format!("Failed to parse response: {}", e)))?;
            
        info!("GCP operation started: {}", operation["name"].as_str().unwrap_or("unknown"));
        
        // Wait for operation to complete
        self.wait_for_operation(&operation["selfLink"].as_str().unwrap_or("")).await?;
        ...
        let instance_response = self.client
            .get(&instance_url)
            .bearer_auth(self.access_token.as_ref().unwrap())
            .send()
            .await
        ...
        let instance: serde_json::Value = instance_response.json().await
            .map_err(|e| Error::ConfigurationError(format!("Failed to parse instance: {}", e)))?;
            
        // Extract IPs
        let network_interface = &instance["networkInterfaces"][0];
        let private_ip = network_interface["networkIP"].as_str().map(|s| s.to_string());
        let public_ip = network_interface["accessConfigs"][0]["natIP"].as_str().map(|s| s.to_string());
    ...
    async fn wait_for_operation(&self, operation_url: &str) -> Result<()> {
        ...
        loop {
            ...
            let response = self.client
                ...
                .send()
                .await
                .map_err(|e| Error::ConfigurationError(format!("Failed to check operation: {}", e)))?;
                
            let operation: serde_json::Value = response.json().await
                .map_err(|e| Error::ConfigurationError(format!("Failed to parse operation: {}", e)))?;
                
            if operation["status"].as_str() == Some("DONE") {
                if let Some(error) = operation.get("error") {
                    return Err(Error::ConfigurationError(format!("Operation failed: {:?}", error)));
                }
                return Ok(());
            }
    ...
    fn map_instance(spec: &ResourceSpec) -> InstanceSelection {
        ...
        let instance_type = match (spec.cpu, spec.memory_gb, gpu_count) {
            ...
            (cpu, mem, _) if mem > cpu * 8.0 => {
                if mem <= 32.0 { "n2-highmem-4" }
                else if mem <= 64.0 { "n2-highmem-8" }
                else { "n2-highmem-16" }
            }
            
            // CPU optimized
            (cpu, mem, _) if cpu > mem / 2.0 => {
                if cpu <= 4.0 { "n2-highcpu-4" }
                else if cpu <= 8.0 { "n2-highcpu-8" }
                else { "n2-highcpu-16" }
            }
...
mod tests {
    ...
    fn test_gcp_instance_mapping() {
        ...
        let result = GcpProvisioner::map_instance(&spec);
        assert!(result.instance_type.contains("standard") || result.instance_type.contains("highcpu"));
