...
impl PricingCalculator {
    ...
    pub fn calculate_cost(
        ...
        for (resource_type, quantity) in &units {
            if let Some(rate) = self.get_resource_rate(resource_type) {
                ...
                resource_costs.insert(resource_type.clone(), ResourceCost {
                    quantity: *quantity,
                    rate_per_unit: rate,
        ...
        let cloud_multiplier = self.cloud_multipliers
            .get(provider)
            .unwrap_or(&1.0);
    ...
    fn get_resource_rate(&self, resource_type: &str) -> Option<f64> {
        self.pricing_config.default.resources.iter()
    ...
    fn load_default_config() -> Result<PricingConfig> {
        ...
        Ok(PricingConfig {
            default: PricingTier {
                resources: vec![
                    ResourcePrice { kind: "CPU".to_string(), count: 1, price_per_unit_rate: 0.001 },
                    ResourcePrice { kind: "MemoryMB".to_string(), count: 1024, price_per_unit_rate: 0.00005 },
                    ResourcePrice { kind: "StorageMB".to_string(), count: 1024, price_per_unit_rate: 0.00002 },
                    ResourcePrice { kind: "NetworkEgressMB".to_string(), count: 1024, price_per_unit_rate: 0.00003 },
                    ResourcePrice { kind: "NetworkIngressMB".to_string(), count: 1024, price_per_unit_rate: 0.00001 },
                    ResourcePrice { kind: "GPU".to_string(), count: 1, price_per_unit_rate: 0.005 },
                ],
            },
...
impl DetailedCostReport {
    ...
    pub fn summary(&self) -> String {
        ...
        summary.push_str(&format!("Base Hourly Cost: ${:.4}\n", self.base_hourly_cost));
        
        if self.cloud_markup > 0.0 {
            summary.push_str(&format!("Cloud Markup: {:.1}%\n", self.cloud_markup * 100.0));
        }
        ...
        summary.push_str(&format!("Final Hourly Cost: ${:.4}\n", self.final_hourly_cost));
        summary.push_str(&format!("Total Cost: ${:.2}\n", self.total_cost));
        summary.push_str(&format!("Monthly Estimate: ${:.2}\n", self.monthly_estimate));
...
mod tests {
    ...
    fn test_provider_comparison() {
        ...
        let generic_report = reports.iter()
            .find(|r| matches!(r.provider, CloudProvider::Generic))
            .unwrap();
        
        let aws_report = reports.iter()
