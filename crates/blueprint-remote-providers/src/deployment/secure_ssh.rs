...
impl SecureSshConnection {
    ...
    pub fn with_port(mut self, port: u16) -> Result<Self> {
        if port == 0 || port > 65535 {
            return Err(Error::ConfigurationError(format!("Invalid SSH port: {}", port)));
        }
        self.port = port;
        Ok(self)
    }
    ...
    fn validate_key_path(path: &Path) -> Result<()> {
        ...
        if path_str.contains("../") || path_str.contains("..\\") {
            return Err(Error::ConfigurationError("Path traversal detected in key path".into()));
        }
        
        if !path.exists() {
            return Err(Error::ConfigurationError(format!(
                "SSH key file does not exist: {}", path.display()
            )));
        }
        ...
        #[cfg(unix)]
        {
            ...
            let metadata = path.metadata()
                .map_err(|e| Error::ConfigurationError(format!("Cannot read key file metadata: {}", e)))?;
            let perms = metadata.permissions().mode();
            
            // SSH keys should be 600 or 400 (owner read/write or read-only)
            if perms & 0o077 != 0 {
                warn!("SSH key file has overly permissive permissions: {:o}", perms);
            }
        }
...
impl SecureSshClient {
    ...
    fn build_secure_ssh_command(&self, command: &str) -> Result<String> {
        ...
        if self.connection.strict_host_checking {
            ...
            if let Some(ref known_hosts) = self.connection.known_hosts_file {
                ssh_cmd.push_str(&format!(" -o UserKnownHostsFile={}", 
    ...
    fn validate_command(&self, command: &str) -> Result<()> {
        if command.is_empty() {
            return Err(Error::ConfigurationError("Empty command not allowed".into()));
        }
    ...
    pub async fn copy_files(&self, local_path: &Path, remote_path: &str) -> Result<()> {
        ...
        info!("Copying files via SCP: {} -> {}", local_path.display(), remote_path);
    ...
    fn build_secure_scp_command(&self, local_path: &Path, remote_path: &str) -> Result<String> {
        ...
        if self.connection.strict_host_checking {
            ...
            if let Some(ref known_hosts) = self.connection.known_hosts_file {
                scp_cmd.push_str(&format!(" -o UserKnownHostsFile={}", 
                    escape(known_hosts.to_str().unwrap().into())));
            }
        } else {
    ...
    fn validate_local_path(&self, path: &Path) -> Result<()> {
        if !path.exists() {
            return Err(Error::ConfigurationError(format!(
                "Local file does not exist: {}", path.display()
            )));
        }
        ...
        if path_str.contains("../") || path_str.contains("..\\") {
            return Err(Error::ConfigurationError("Path traversal detected in local path".into()));
        }
...
mod tests {
    ...
    fn test_secure_ssh_connection_validation() {
        ...
        // Invalid hostname
        assert!(SecureSshConnection::new("host; rm -rf /".to_string(), "user".to_string()).is_err());
        
        // Invalid username
        assert!(SecureSshConnection::new("example.com".to_string(), "user; id".to_string()).is_err());
    }
