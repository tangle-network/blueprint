...
impl PasetoTokenManager {
    ...
    pub fn generate_token(
        ...
        let custom_json = serde_json::to_string(&claims)
            .map_err(|e| PasetoError::SerializationError(e.to_string()))?;
        paseto_claims.add_additional("data", custom_json)
            .map_err(|e| PasetoError::SerializationError(e.to_string()))?;
    ...
    pub fn validate_token(&self, token: &str) -> Result<AccessTokenClaims, PasetoError> {
        ...
        let trusted_token = pasetors::local::decrypt(&self.key.0, &untrusted_token, &validation_rules, None, None)
            .map_err(|e| PasetoError::DecryptionError(e.to_string()))?;
        
        // Extract our custom data from the "data" field
        let token_claims = trusted_token.payload_claims()
            .ok_or_else(|| PasetoError::DeserializationError("Failed to parse payload claims".to_string()))?;
            
        let custom_data = token_claims.get_claim("data")
            .ok_or_else(|| PasetoError::DeserializationError("Missing data claim".to_string()))?;
        
        // Get string value from the claim
        let custom_json_str = custom_data.as_str()
            .ok_or_else(|| PasetoError::DeserializationError("Data claim is not a string".to_string()))?;
