...
use crate::db::{cf, RocksDb};
use crate::models::{TlsProfile, TlsCertMetadata};
...
impl TlsAssetManager {
    ...
    pub fn store_tls_profile(&self, service_id: u64, profile: TlsProfile) -> Result<(), crate::Error> {
    ...
    pub fn store_cert_metadata(&self, cert_id: &str, metadata: TlsCertMetadata) -> Result<(), crate::Error> {
    ...
    pub fn get_cert_metadata(&self, cert_id: &str) -> Result<Option<TlsCertMetadata>, crate::Error> {
        let key = format!("cert_metadata:{}", cert_id);
        let cf_handle = self.db.cf_handle(cf::TLS_CERT_METADATA_CF).unwrap();
    ...
    pub fn list_cert_metadata(&self, service_id: u64) -> Result<Vec<TlsCertMetadata>, crate::Error> {
    ...
    pub fn store_encrypted_cert(&self, cert_id: &str, cert_data: &[u8]) -> Result<(), crate::Error> {
        let key = format!("cert_data:{}", cert_id);
        let encrypted_data = self.tls_envelope.encrypt(cert_data)?;
        let cf_handle = self.db.cf_handle(cf::TLS_ASSETS_CF).unwrap();
        
        self.db.put_cf(&cf_handle, key.as_bytes(), &encrypted_data)?;
        Ok(())
    }
    ...
    pub fn store_encrypted_private_key(&self, key_id: &str, key_data: &[u8]) -> Result<(), crate::Error> {
    ...
    pub fn store_encrypted_ca_bundle(&self, bundle_id: &str, bundle_data: &[u8]) -> Result<(), crate::Error> {
    ...
    pub fn get_encrypted_ca_bundle(&self, bundle_id: &str) -> Result<Option<Vec<u8>>, crate::Error> {
        let key = format!("ca_bundle_data:{}", bundle_id);
        let cf_handle = self.db.cf_handle(cf::TLS_ASSETS_CF).unwrap();
    ...
    pub fn get_issuance_logs(&self, start_time: u64, end_time: u64) -> Result<Vec<String>, crate::Error> {
        let mut result = Vec::new();
        let cf_handle = self.db.cf_handle(cf::TLS_ISSUANCE_LOG_CF).unwrap();
    ...
    pub fn validate_certificate_chain(&self, cert_data: &[u8], ca_bundle_data: &[u8]) -> Result<bool, crate::Error> {
    ...
    pub fn cleanup_expired_certificates(&self) -> Result<u32, crate::Error> {
        ...
        for item in self.db.prefix_iterator_cf(&metadata_cf_handle, b"cert_metadata:") {
            let (key, value) = item?;
