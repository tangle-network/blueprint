...
mod tests {
    ...
    async fn test_api_key_cross_user_isolation() {
        ...
        let users = vec![
            ("alice@company.com", "alice_hash", "Alice Corp"),
            ("bob@company.com", "bob_hash", "Bob Corp"),
            ("eve@malicious.com", "eve_hash", "Evil Corp"),
        ];
        ...
        for (email, tenant_hash, company, signing_key, public_key) in user_data {
            ...
            user_api_keys.push((email, tenant_hash, api_key));
        }
    ...
    async fn test_token_exchange_header_injection_security() {
        ...
        for (header_name, header_value) in malicious_headers {
            ...
            let exchange_req = TokenExchangeRequest {
                additional_headers: attack_headers,
                ttl_seconds: Some(60),
            };
            ...
            if matches!(
                ...
                // Other headers might be allowed but shouldn't grant additional privileges
                if res.status().is_success() {
                    let _: TokenExchangeResponse = res.json().await;
                    // Token should still contain only legitimate tenant info
                    // (The actual privilege verification would happen in the resource server)
                }
            }
        }
    }
