...
impl ClientCertExtension {
    ...
    pub fn additional_headers(&self) -> HeaderMap {
        ...
        if let Some(cert) = &self.client_cert {
            ...
            headers.insert("x-client-cert-not-before", cert.not_before.to_string().parse().unwrap());
            headers.insert("x-client-cert-not-after", cert.not_after.to_string().parse().unwrap());
