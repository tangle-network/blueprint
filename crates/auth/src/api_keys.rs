...
impl ApiKeyGenerator {
    ...
    pub fn generate_key<R: RngCore + CryptoRng>(
        ...
        let key_id = format!("{}{}", self.prefix, CUSTOM_ENGINE.encode(&secret_bytes[..8]));
...
impl ApiKeyModel {
    ...
    pub fn find_by_id(id: u64, db: &RocksDb) -> Result<Option<Self>, Error> {
        ...
        if let Some(key_id_bytes) = db.get_pinned_cf(&cf, id.to_be_bytes())? {
            let key_id = String::from_utf8(key_id_bytes.to_vec())
                .map_err(|_| Error::UnknownKeyType)?; // Reusing error type
    ...
    fn create(&mut self, db: &RocksDb) -> Result<(), Error> {
        ...
        loop {
            ...
            match result {
                Ok(()) => break,
                Err(e) if matches!(e.kind(), rocksdb::ErrorKind::Busy | rocksdb::ErrorKind::TryAgain) => {
                    retry_count += 1;
...
mod tests {
    ...
    fn test_api_key_database_operations() {
        ...
        let found_by_id = ApiKeyModel::find_by_id(model.id, &db)
            .unwrap()
            .unwrap();
        ...
        assert!(ApiKeyModel::find_by_key_id(&key.key_id(), &db).unwrap().is_none());
    }
