//! Integration test for the TangleProducer backed by the Anvil harness.
//!
...
use blueprint_tangle_extra::extract::{CallId, ServiceId, TangleArg};
...
async fn tangle_producer_streams_real_job_submissions() -> Result<()> {
    ...
    run_anvil_test("tangle_producer_streams_real_job_submissions", async {
        ...
        let producer = TangleProducer::from_block(client.clone(), LOCAL_SERVICE_ID, start_block)
        ...
        let TangleArg((doc_id, content, account)) =
            TangleArg::<(String, String, String)>::from_job_call(job_call, &())
                .await
...
async fn create_client(deployment: &SeededTangleTestnet) -> Result<TangleClient> {
...
async fn submit_job(client: &TangleClient) -> Result<()> {
...
async fn log_service_status(client: &TangleClient) -> Result<()> {
    let service = client
