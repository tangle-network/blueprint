use blueprint_qos::{
    ...
    default_qos_config,
    QoSServiceBuilder,
    heartbeat::{HeartbeatConsumer, Heartbeat, HeartbeatConfig},
    metrics::MetricsConfig,
    logging::{LokiConfig, GrafanaConfig},
...
mod tests {
    ...
    async fn test_qos_service_builder_requires_consumer() {
        ...
        if let Err(QosError::Other(msg)) = result {
            ...
            panic!("Expected Other error for missing consumer, got {:?}", result);
        }
    }
    ...
    async fn test_qos_service_build_with_default_config() {
        ...
        let service_result = QoSServiceBuilder::new()
            ...
            .build()
            .await;
        assert!(service_result.is_ok(), "Service build failed: {:?}", service_result.err());
        let service = service_result.unwrap();
        
        let (has_heartbeat, has_metrics, has_grafana) = check_service_components(&service);
        assert!(!has_heartbeat, "Heartbeat service should be None with default QoSConfig");
        assert!(!has_metrics, "Metrics service should be None with default QoSConfig");
        assert!(!has_grafana, "Grafana client should be None with default QoSConfig");
    }
    ...
    async fn test_qos_service_builder_individual_component_configs() {
        ...
        let service_hb_res = QoSServiceBuilder::new()
            ...
            .build()
            .await;
        assert!(service_hb_res.is_ok(), "HB service build failed: {:?}", service_hb_res.err());
        ...
        let service_metrics_res = QoSServiceBuilder::new()
            ...
            .build()
            .await;
        assert!(service_metrics_res.is_ok(), "Metrics service build failed: {:?}", service_metrics_res.err());
        let service_metrics = service_metrics_res.unwrap();
        let (has_heartbeat_metrics, has_metrics_metrics, _) = check_service_components(&service_metrics);
        ...
        let service_grafana_res = QoSServiceBuilder::new()
            ...
            .build()
            .await;
        assert!(service_grafana_res.is_ok(), "Grafana service build failed: {:?}", service_grafana_res.err());
        ...
        let service_loki_res = QoSServiceBuilder::new()
            ...
            .build()
            .await;
        assert!(service_loki_res.is_ok(), "Loki service build failed: {:?}", service_loki_res.err());
    }
}
