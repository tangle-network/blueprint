...
struct RunnerTestHarness {
    ...
    runner_client: Arc<TangleClient>,
    control_client: Arc<TangleClient>,
    start_block: u64,
    _temp_dir: TempDir,
...
impl RunnerTestHarness {
    ...
    fn router(&self) -> Router {
        Router::new().route(JOB_INDEX, echo_job.layer(TangleLayer))
    }

    fn producer(&self) -> TangleProducer {
        TangleProducer::from_block((*self.runner_client).clone(), SERVICE_ID, self.start_block)
            .with_poll_interval(Duration::from_millis(100))
    }

    fn consumer(&self) -> TangleConsumer {
        TangleConsumer::new((*self.runner_client).clone())
    }

    fn control_client(&self) -> Arc<TangleClient> {
