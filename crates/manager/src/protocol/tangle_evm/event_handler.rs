...
use crate::protocol::tangle_evm::metadata::OnChainMetadataProvider;
...
/// Handles Tangle EVM events and translates them into blueprint lifecycle actions.
pub struct TangleEvmEventHandler {
    metadata: Arc<dyn BlueprintMetadataProvider>,
}

impl TangleEvmEventHandler {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub async fn initialize(
        &self,
        client: &mut TangleEvmProtocolClient,
        ...
        if let Some(evt) = client.client().latest_event().await {
            info!(
                "Tangle EVM client initialized at block {}",
                evt.block_number
            );
        }
        Ok(())
    ...
    pub async fn handle_event(
        ...
        let tangle_evt = event
            .as_tangle_evm()
            .ok_or_else(|| Error::Other("Expected Tangle EVM event in handler".to_string()))?;
...
impl Default for TangleEvmEventHandler {
    fn default() -> Self {
        Self::new()
    }
}
