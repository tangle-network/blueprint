...
Producer/Consumer extras for Tangle EVM (v2) blueprints with BLS signature aggregation support.

## Overview

This crate provides the infrastructure for building blueprints that interact with Tangle v2 EVM contracts, including:

- **TangleEvmProducer**: Polls for `JobSubmitted` events and streams job calls
- **TangleEvmConsumer**: Submits individual job results via `submitResult`
...
use blueprint_tangle_evm_extra::{TangleEvmConsumer, TangleEvmProducer, TangleEvmLayer};
...
async fn main() -> Result<(), blueprint_sdk::Error> {
    ...
    BlueprintRunner::builder(TangleEvmConfig::default(), env)
        .router(Router::new()
            .route(0, my_job.layer(TangleEvmLayer)))
...
```
              ...
              │                             │
    ┌─────────▼─────────┐         ┌────────▼────────┐
    │ TangleEvmConsumer │         │AggregatingConsumer│
