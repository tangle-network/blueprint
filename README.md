...
> **Note:** The harness loads `crates/chain-setup/anvil/snapshots/localtestnet-state.json` (override via `ANVIL_SNAPSHOT_PATH`) and only falls back to replaying `LocalTestnet.s.sol` when the snapshot is missing or fails validation. Keep a sibling `tnt-core` checkout via `TNT_CORE_PATH=/path/to/tnt-core` so the fallback path works, set `RUN_TNT_E2E=1` to opt into the longer suites, and export `BLUEPRINT_ANVIL_LOGS=1` if you want to stream Anvil’s stdout/stderr while debugging.
